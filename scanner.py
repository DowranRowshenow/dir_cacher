...
def _get_dll_path():
    ...
    # Portable app layout (THIS IS YOUR MAIN CASE)
    base_dir = os.path.dirname(os.path.abspath(__file__))
    portable_path = os.path.join(base_dir, dll_name)

    if os.path.exists(portable_path):
        return portable_path

    # Dev fallback (Rust target)
    dev_path = os.path.join(base_dir, "scanner_core", "target", "release", dll_name)
    if os.path.exists(dev_path):
        return dev_path

    raise FileNotFoundError(f"scanner_core DLL not found: {dll_name}")
...
try:
    ...
    ENTRY_CALLBACK = CFUNCTYPE(
        None, c_char_p, c_char_p, c_char_p, c_int, c_int64, c_double
    )
    _scanner_lib.scan_directory.argtypes = [
        c_char_p,
        c_void_p,
        c_void_p,
        ENTRY_CALLBACK,
        ...
        try:
            ...
            if not _scanner_lib:
                ...
                for root_dir in self.root_paths:
                    ...
                    _scanner_lib.scan_directory(
                        c_root, self._cancel_flag, self._pause_flag, c_callback
                    )
